...
% %  movname='LcutR1-2chamber';
% %  movname='non-2chamber';
% %   stim_box='right';
%   movname='l1_2chamber';
%   stim_box='left';
...
if plot_bound==1
    ...
    if do_tracking==1
        ...
        elseif strcmp(movname,'new_context_d2')
            ...
            x_stimbound=290.2696-xbound(1);
            shift=0;
        elseif  strcmp(movname,'LcutR1-2chamber')
               ybound=[101,327];
            xbound=[167,517];
            x_stimbound=343.3571-xbound(1);
            shift=0;
            elseif  strcmp(movname,'non-2chamber')
               ybound=[79,304];
            xbound=[167,517];
            x_stimbound= 341.8825-xbound(1);
            shift=0;
...
%%
                ...
                if ~isempty(ia_start)
                   ...
                   for i=1:length(ia_start)
                        [~,min_ia(i)]=min(a(ia_low(ia_start(i):ia_end(i))));
                   end
                     for j=1:length(ib_start)
                          [~,min_ib(j)]=min(b(ib_low(ib_start(j):ib_end(j))));
                     end
                     for i=1:length(min_ia)
                         for j=1:length(min_ib)
%                             area_test(i,j)=mean(mean(video(max([1,min_ib(j)+4-area_r]):min([size(video,1),min_ib(j)+4+area_r]),max([1,min_ia(i)+4-area_r]):min([size(video,2),min_ia(i)+4+area_r]))));
                         area_test(i,j)=mean(mean(bw(max([1,min_ib(j)-area_r]):min([size(bw,1),min_ib(j)+area_r]),max([1,min_ia(i)-area_r]):min([size(bw,2),min_ia(i)+area_r]))));
                          end
                     end
                 
                     tmp=find(area_test==max(max(area_test)));
                         ib(count)=tmp(1);
                         ia(count)=tmp(2);
%                 clear bw_a
%                 for i=1:length(ia_start)                  
...
%                 ib(count)=round([ib_low(ib_start(bwsel_b))+ib_low(ib_end(bwsel_b))]/2);
                ...
                %%
                if check_area==1
                    area=mean(mean(bw(max([1,ib(count)-area_r]):min([size(bw,1),ib(count)+area_r]),max([1,ia(count)-area_r]):min([size(bw,2),ia(count)+area_r]))));
                    if area>=start  && count>1
                        if do_test
                            %                 imagesc(video(ybound(1):ybound(2),xbound(1):xbound(2),:));
                            imshow(video);%axis xy;
                            hold on,
                            plot(ia(count)+4,ib(count)+4,'r^');
                            title('pre');
                            %                  plot(ib(count),ia(count),'r^');
                            hold off
                            pause
