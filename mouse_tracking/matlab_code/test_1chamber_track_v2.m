...
if plot_bound==1
    ...
    if do_tracking==1        
        ...
        for count=1:tcount
            ...
            if do_extract_background==1 && shift+count*sample_int-5/V.FrameRate>0
                %             background=extract_background(movfullname);
                
                V_pre=VideoReader(movfullname,'CurrentTime',shift+count*sample_int-5/V.FrameRate);
                video_pre=pre_weight(1)*readFrame(V_pre);
                for i=2:5
                    video_pre=video_pre+pre_weight(i)*readFrame(V_pre);
                end
                video_pre=video_pre/sum(pre_weight);
                video=video+video_pre (ybound(1):ybound(2),xbound(1):xbound(2));
            end
            ...
            if count>1 && sqrt((ia(count)-ia(count-1))^2+(ib(count)-ib(count-1))^2)>maxspeed*sample_int
